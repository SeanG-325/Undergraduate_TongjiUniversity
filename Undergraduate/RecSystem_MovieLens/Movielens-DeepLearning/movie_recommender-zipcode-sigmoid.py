...
def get_user_feature_layer(uid_embed_layer, gender_embed_layer, age_embed_layer, job_embed_layer, area_embed_layer):
    with tf.name_scope("user_fc"):
        ...
        #第二层全连接
        user_combine_layer = tf.concat([uid_fc_layer, gender_fc_layer, age_fc_layer, job_fc_layer, area_fc_layer, area_fc_layer], 2)  #(?, 1, 160)
        user_combine_layer = tf.contrib.layers.fully_connected(user_combine_layer, 200, tf.sigmoid)  #(?, 1, 200)
...
def get_movie_feature_layer(movie_id_embed_layer, movie_categories_embed_layer, dropout_layer):
    with tf.name_scope("movie_fc"):
        ...
        movie_combine_layer = tf.contrib.layers.fully_connected(movie_combine_layer, 200, tf.sigmoid)  #(?, 1, 200)
