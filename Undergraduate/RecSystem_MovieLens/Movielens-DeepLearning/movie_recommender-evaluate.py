...
print(float(rating_movie(1, 2355)[0]))
...
def recommend_same_type_movie(movie_id_val, top_k = 20, PrintOrNot = True):
...
def recommend_your_favorite_movie(user_id_val, top_k = 10, PrintOrNot = True):
    ...
    with tf.Session(graph=loaded_graph) as sess:  #
        ...
        k=sim.tolist()
        #print(k)
        temp=np.where(movies_orig==2799)
        print(int(temp[0]))
        print(-sim[0][int(temp[0])])
...
recommend_your_favorite_movie(161, 10)
