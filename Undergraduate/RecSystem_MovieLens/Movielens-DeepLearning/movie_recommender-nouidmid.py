...
def get_user_embedding(uid, user_gender, user_age, user_job):
    ...
    return gender_embed_layer, age_embed_layer, job_embed_layer


# #### 将User的嵌入矩阵一起全连接生成User的特征

#def get_user_feature_layer(uid_embed_layer, gender_embed_layer, age_embed_layer, job_embed_layer):
def get_user_feature_layer(gender_embed_layer, age_embed_layer, job_embed_layer):
    with tf.name_scope("user_fc"):
        ...
        user_combine_layer = tf.concat([gender_fc_layer, age_fc_layer, job_fc_layer], 2)
        user_combine_layer = tf.contrib.layers.fully_connected(user_combine_layer, 100, tf.tanh)  #(?, 1, 200)
    
        user_combine_layer_flat = tf.reshape(user_combine_layer, [-1, 100])
...
def get_movie_feature_layer(movie_id_embed_layer, movie_categories_embed_layer, dropout_layer):
    with tf.name_scope("movie_fc"):
        ...
        movie_combine_layer = tf.concat([movie_categories_fc_layer, dropout_layer], 2)  #(?, 1, 96)
        movie_combine_layer = tf.contrib.layers.fully_connected(movie_combine_layer, 100, tf.tanh)  #(?, 1, 200)
    
        movie_combine_layer_flat = tf.reshape(movie_combine_layer, [-1, 100])
...
with train_graph.as_default():
    ...
    gender_embed_layer, age_embed_layer, job_embed_layer = get_user_embedding(uid, user_gender, user_age, user_job)
    #得到用户特征
    #user_combine_layer, user_combine_layer_flat = get_user_feature_layer(uid_embed_layer, gender_embed_layer, age_embed_layer, job_embed_layer)
    user_combine_layer, user_combine_layer_flat = get_user_feature_layer(gender_embed_layer, age_embed_layer, job_embed_layer)
    #获取电影ID的嵌入向量
...
#     test = loaded_graph.get_tensor_by_name("test/test/BiasAdd:0")
    ...
    return uid, user_gender, user_age, user_job, movie_id, movie_categories, movie_titles, targets, lr, dropout_keep_prob, test, movie_combine_layer_flat, user_combine_layer_flat
...
def rating_movie(user_id_val, movie_id_val):
    ...
    with tf.Session(graph=loaded_graph) as sess:  #
        ...
        uid, user_gender, user_age, user_job, movie_id, movie_categories, movie_titles, targets, lr, dropout_keep_prob, test,_, __ = get_tensors(loaded_graph)  #loaded_graph
